...
// 获取所有分类 https://api.yujiameimei.com/douyin/dy.php?getsort

// 获取某个分类下视频列表   https://api.yujiameimei.com/douyin/dy.php?sort=/guochanzipai/&page=1

// 搜索  https://api.yujiameimei.com/douyin/dy.php?keyword=美女&page=3

// 获取视频详情  https://api.yujiameimei.com/douyin/dy.php?id=/guochanzipai/25057/
...
if(isset($_GET['getsort']) ) {
	...
	//构建目标链接
	$sourceUrl = $latestdomain . "/video/";
    // 获取源码
    //$sourceCode = file_get_contents($sourceUrl);
    $sourceCode = file_get_contents($sourceUrl,false,$context);
	
    // 利用正则表达式截取文本
    preg_match('/type-slide m-bottom-line(.*?)<\/dd>/s', $sourceCode, $matches);
    ...
    if (isset($matches[1])) {
		...
		//$image = isset($imageMatches[1]) ? ( $domain . $imageMatches[1]) : '';
           ...
           // preg_match('/href="(.*?)"/', $category, $idMatches);
            ...
            // 提取标题
           preg_match('/<a href="(.*?)">(.*?)<\//s', $category, $matches);
   ...
   // $sourceCode = str_replace('rel="nofollow"><img class=','',$sourceCode);
	// 利用正则表达式截取文本
	preg_match('/stui-pannel_bd">(.*?)stui-page/s', $sourceCode, $matches);
...
$page = isset($_GET['page']) ? intval($_GET['page']) : 1;
    ...
    // 构建目标网站的 URL
    $sourceUrl = $latestdomain . "/index.php/vod/search/page/$page.html"; 
...
preg_match('/stui-pannel_bd(.*?)<\/ul>/s', $sourceCode, $matches);
...
if (isset($matches[1])) {
    ...
    $videoId = isset($_GET['id']) ? $_GET['id'] : '';
        ...
        // 构建目标网站的 URL
        $sourceUrl = $latestdomain  .  $videoId . "index_1_1.html";
        // 获取源码
        $sourceCode = file_get_contents($sourceUrl,false,$context);
