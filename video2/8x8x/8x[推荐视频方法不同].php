...
function getRedirectUrl($initialUrl) {
	...
	$videodomain = "https://avxsgtfj.xyz:32768/v/";
...
// 将文本a中的第一和第二个“<a href=”替换为空
   // $sourceCode = preg_replace('/<li><a/', '', $sourceCode, 1);
       ...
       // $jsurl = isset($jsurlMatches[1]) ? $jsurlMatches[1] : '';
        ...
        $videoUrl1 = isset($videoMatches[1]) ? str_replace("\\","",$videoMatches[1]) : '';
    
	    //$videoUrl =$jsurl . $videoUrl1;
		$videoUrl =$videodomain . $videoUrl1;
       ...
       // }
        ...
        // 获取推荐视频列表
        preg_match('/换一批视频(.*?)<footer>/s', $sourceCode, $matches);
        if (isset($matches[1])) {
