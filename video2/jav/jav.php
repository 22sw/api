...
// 利用正则表达式截取文本
    preg_match('/<div class="itemimg item-0(.*?)<\/section>/s', $sourceCode, $matches);
...
// 判断是否存在GET请求参数
    ...
    $page = $_GET['page'];
        ...
        $url = $latestdomain .$category."&pg=" .$page;
...
//echo $content;
		...
		$image = isset($imageMatches[1]) ? $imageMatches[1] : '';
          ...
          preg_match('/<a href="(.*?)"/s', $category, $idmatches);
		   ...
		   // 提取标题
           preg_match('/195"\/>(.*?)</s', $category, $titlematches);
    ...
    $page = $_GET['page'];
      ...
      $url = $latestdomain ."/so.php?url=$keyword&pg=$page" ;
...
// 将文本a中的第一和第二个“<a href=”替换为空
    //$sourceCode = preg_replace('/url/', '', $sourceCode, 2);
        ...
        // 从源码中截取标题
        preg_match('/正在觀看：(.*?)<\/title>/s', $sourceCode, $titleMatches);
        $title = isset($titleMatches[1]) ? $titleMatches[1] : '';
