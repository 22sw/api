...
// https://api.22ba4.top/xv/tag.php?month=2024-02&page=2
//  获取日期 https://api.22ba4.top/xv/best.php?getdate

// 获取参数
    //$sort = isset($_GET['sort']) ? $_GET['sort'] : '';
	$month = isset($_GET['month']) ? $_GET['month'] : '';
    $page = isset($_GET['page']) ? intval($_GET['page'])-1 : 1;
    $getdate = isset($_GET['getdate']) ? $_GET['getdate'] : '';
...
if(isset( $_GET['month'] && $_GET['page']) {


// 构建目标网站的 URL
   $sourceUrl = "https://www.xvideos.com/best/$month/$page";
...
} elseif (isset($_GET['getdate'])) {

// 构建目标网站的 URL
   $sourceUrl = "https://www.xvideos.com/best/2008-01/2";
...
// 利用正则表达式截取文本
preg_match('/label-list"><ul>(.*?)<\/ul>/s', $sourceCode, $matches);
...
if (isset($matches[1])) {
  ...
  //  array_pop($videos);
    ...
    foreach ($videos as $video) {
        // 提取标题
        preg_match('/default">(.*?)<\/a/', $video, $titleMatches);
        $title = isset($titleMatches[1]) ? $titleMatches[1] : '';
