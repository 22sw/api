...
function getRedirectedUrl($url, $cacheFile, $cacheTime = 600) { // 10分钟 = 600秒
	...
	$latestdomain ="https://www.book18.org";
...
if (isset($_GET['getsort'])) {
    ...
    $sourceUrl = "$latestdomain/article/index";
   
    $sourceCode = file_get_contents($sourceUrl);

    // 利用正则表达式截取文本
    preg_match('/<\/ul><ul(.*?)nav-header/s', $sourceCode, $matches);
    ...
    if (isset($matches[1])) {
		...
		//$image = isset($imageMatches[1]) ? ( $domain . $imageMatches[1]) : '';
           ...
           // preg_match('/href="(.*?)"/', $category, $idMatches);
            ...
            // 提取标题
           preg_match('/href="(.*?)"><i class=\'(.*?)\'><\/i> <p>(.*?)<\/p>/s', $category, $matches);
    ...
    $page = isset($_GET['page']) ? intval($_GET['page']) : 1;
     ...
     // 构建目标网站的 URL
$sourceUrl = "$latestdomain{$category}?page={$page}&per-page=100";
   
    // 获取源码
	$sourceCode = file_get_contents($sourceUrl,false,$context);

	// 利用正则表达式截取文本
	preg_match('/col-md-6(.*?)pagination/s', $sourceCode, $matches);
...
$sourceUrl = "$latestdomain/q/{$keyword}?page={$page}"; 
 ...
 //echo $sourceCode;
	// 利用正则表达式截取文本
	preg_match('/row links(.*?)pagination/s', $sourceCode, $matches);
...
if (isset($matches[1])) {
	...
	// 提取时长
        preg_match('/<a href="(.*?)" target="_self"><b class="mr-1">(.*?)<\/b>/s', $video, $descriptionMatches);
        $id = isset($descriptionMatches[1]) ? $descriptionMatches[1] : '';
		$title = isset($descriptionMatches[2]) ? $descriptionMatches[2] : '';
   ...
   if (strpos($id,"book") !==false){
	   $sourceCode = file_get_contents($sourceUrl );
	   preg_match('/px-2"><a href="(.*?)"/s', $sourceCode, $code1Matches);
       $code1 = isset($code1Matches[1]) ? $code1Matches[1] : '';
	   $sourceUrl = $latestdomain . $code1;
   ...
   preg_match('/hidden;max-width:1000px;"><p>(.*?)<\/div><div class="row no-gutters m-0/s', $sourceCode, $code2Matches);
   $novelCode = isset($code2Matches[1]) ? str_replace("book18.org","", $code2Matches[1]): '';
	   $novelCode = '<style>p { font-size: 25px; }</style>' . $novelCode;
