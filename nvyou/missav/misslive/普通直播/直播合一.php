...
if(isset($_GET['primaryTag']) && isset($_GET['parentTag'])&& isset($_GET['page'])) {
// 获取主标签、筛选标签和父标签参数
...
if (!empty($data['models'])) {
    ...
    foreach ($data['models'] as $key => &$model) {
        ...
        // 只保留指定键
        $model = array_intersect_key($model, array_flip(['gender', 'avatarUrl', 'country', 'previewUrlThumbBig', 'previewUrlThumbSmall', 'username', 'hlsPlaylist', 'status', 'viewersCount', 'id', 'isOnline', 'topic', 'activity']));
    }
