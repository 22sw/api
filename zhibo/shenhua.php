...
// 解析 GET 请求参数
// 判断是否传入了参数
    $latestdomain ="https://api.zbjk.live";
	
	
if (isset($_GET['getpingtai'])  ) {
	 
	
	$sourceUrl = $latestdomain . "/sh/json.txt";
...
// 判断是否存在GET请求参数
    ...
    $page = $_GET['page'];
        ...
        $sourceUrl = $latestdomain .  "/sh/".$pingtai;
        
         // 获取源码
    $sourceCode = file_get_contents($sourceUrl);

    // 解密秘钥
$key = "abs78def1121ghag";
...
$decryptedData = decryptData(base64_decode($sourceCode), $key);
    ...
    if (!empty($dataArray['zhubo'])) {
        ...
        foreach ($dataArray['zhubo'] as $video) {
            // 修改键名
			if(
			  strpos($video['address'],"xrtpor.com")===false  &&  strpos($video['address'],"irpo.cn")===false  &&  
			    strpos($video['address'],"lxhyouth.xyz")===false  &&       strpos($video['address'],"aliyuncs.com")===false  &&  
			    strpos($video['address'],"liuzhuan.xyz")===false  &&    strpos($video['address'],".mp4")===false  &&
				strpos($video['address'],"guwbanb.cn")===false  &&    strpos($video['address'],"myqcloud.com")===false  &&  
				strpos($video['address'],"cx_2882.flv")===false  &&    strpos($video['address'],"ddbofh.cn")===false  &&   
				strpos($video['title'],"放映")===false
