...
$context = stream_context_create([
    ...
    $latestdomain = 'https://apiaws.vmcbh.com';
    $token = 'token=eyJ1c2VyX2lkIjo1MzEwNTM0NDksImxhc3Rsb2dpbiI6MTcxNDYzNzI3MH0.c574ce2f3a18e471c926047af1daace4.19e1fe2a7e97858edf3c1333b21fc01308fb2fc9aae0491973e79a91';
	$newurl =  'https://apiaws.vmcbh.com';
...
if (!empty($dataArray['response']['categories']['long'])) {
    ...
    foreach ($dataArray['response']['categories']['long'] as $video) {
        ...
        $videoItem = [
			...
			'id' => 'long' . '$category=' . $video['real_category'] 
        ];
        // 添加到视频数组中
        $videos[] = $videoItem;
    }
...
if (!empty($dataArray['response']['categories']['short'])) {
    foreach ($dataArray['response']['categories']['short'] as $video) {
        ...
        $videoItem = [
            ...
            'image' => $video['category_image'],
            'real_category' => $video['real_category'],
			'id' => 'short' . '$category=' . $video['real_category'] 
        ];
        // 添加到视频数组中
        $videos[] = $videoItem;
    }
...
$response['categories'] = $videos;
   ...
   // if ($page == 1) {
		...
		//$sourceUrl = "https://apih.yaowanzhuan.net/pwa/videos/list?page=1&token=eyJ1c2VyX2lkIjo1MzEwNTM0NDksImxhc3Rsb2dpbiI6MTcxNDYzNzI3MH0.c574ce2f3a18e471c926047af1daace4.19e1fe2a7e97858edf3c1333b21fc01308fb2fc9aae0491973e79a91&video_type=long&category=美腿";
...
if (!empty($dataArray['response']['videos'])) {
    ...
    foreach ($dataArray['response']['videos'] as $video) {
        ...
        $videoItem = [
            ...
            'image' =>"https://encimgt.yaowanzhuan.net" .  $video['video_thumb'],
            'views' => $video['video_views'],
            'date' => date("Y-m-d", $video['video_upload_date']),
    ...
    // 构建目标网站的 URL
    //if ($page == 1) {
        $sourceUrl = "$latestdomain/pwa/videos/long/search?keyword={$keyword}&order=1&{$token}&page=$page";
...
$id = isset($_GET['id']) ? $_GET['id'] : '';
    ...
    // $sourceUrl =  "https://apiaws.vmcbh.com/pwa/video/info/240434?token=eyJ1c2VyX2lkIjo1MzEwNTM0NDksImxhc3Rsb2dpbiI6MTcxNDYzNzI3MH0.c574ce2f3a18e471c926047af1daace4.19e1fe2a7e97858edf3c1333b21fc01308fb2fc9aae0491973e79a91";
...
if (!empty($dataArray['response'])) {
    ...
    $response['image'] ="https://encimgt.yaowanzhuan.net" .  $dataArray['response']['thumb'];
    if (isset($dataArray['response']['video_urls']['intro'])) {
        $response['video'] = 'https://streamf.topert.net' . $dataArray['response']['video_urls']['intro'] . '?' . $token;
    }else{
		$response['video'] = "";
	}
...
//$sourceCode = 'yKSLyOeV2xS/KJCeGPudxUeVVU9ayDFU/1KW5C0tH+X4TfrblXvtbVUi2dzDTQi0L8YpCYRoEnQsDa+e6uWOSg==';
//$aaa = decryptAES($sourceCode, 'ae52f7ffd2dd66ba5743bb180188b991', '8f00b204e9800998');
//echo $aaa;
