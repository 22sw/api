...
if (isset($_GET['getsort'])) {
	...
	$sourceUrl = "$latestdomain/json/zone_7.json?$str_time";
...
$dataArray = json_decode($sourceCode, true);
    ...
    $sourceUrl2 = "$latestdomain/json/zone_8.json?$str_time";
...
if (!empty($dataArray2['repository_zone_1001'])) {
    foreach ($dataArray2['repository_zone_1001'] as $video) {
        // 构建视频项的关联数组
        $videoItem = [
            'categorie' => 'AV大全',
            'id' => $video['repositoryZoneId'],
...
if (!empty($dataArray2['repository_zone_1002'])) {
    foreach ($dataArray2['repository_zone_1002'] as $video) {
        // 构建视频项的关联数组
        $videoItem = [
            'categorie' => '香蕉专区',
            'id' => $video['repositoryZoneId'],
...
$response['sort']='高分色情-资深狼友-撸视频-刷福利';
$response['高分色情'] = $videosshort;
$response['资深狼友'] = $videos;
$response['撸视频'] = $videos3;
$response['刷福利'] = $videos4;
