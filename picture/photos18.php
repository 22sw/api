...
function getRedirectUrl($hostUrl) {
	...
	$latestdomain="https://www.photos18.com";
...
if(isset($_GET['getsort']) ) {
    ...
    // 利用正则表达式截取文本
    preg_match('/nav-header(.*?)<\/aside>/s', $sourceCode, $matches);
 ...
 //echo $content;
   ...
   // $sourceCode = str_replace('target="_blank"><','',$sourceCode);
	// 利用正则表达式截取文本
	preg_match('/card-columns(.*?)pagination/s', $sourceCode, $matches);
...
if (isset($matches[1])) {
    ...
    foreach ($videos as $video) {
        ...
        // 提取标题
        preg_match('/<a href="(.*?)">(.*?)<\/a>/s', $video, $titleMatches);
        $title = isset($titleMatches[2]) ? $titleMatches[2] : '';
		
		
	
	// 提取更新日期
        preg_match('/badge-primary">(.*?)</s', $video, $durationMatches);
        $zhangjie = isset($durationMatches[1]) ? trim($durationMatches[1]) : '';
