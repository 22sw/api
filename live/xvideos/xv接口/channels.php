...
// 最佳 获取频道筛选（mian regoin rank）   https://api.22ba4.top/xv/channels.php?data
// 频道获取列表 mian regoin rank page  https://api.22ba4.top/xv/channels.php?main=/channels-index&region=/china&rank=/from/worldwide&page=1
// 某个频道视频列表  channels  page  https://api.22ba4.top/xv/channels.php?channels=/enjoypornpass&page=2
...
if (isset($matches[1])) {
   ...
   // $content = preg_replace('/<a href="/', '', $content, 2);
    ...
    foreach ($videos as $video) {
        ...
        // 提取标题
        preg_match('/#(\d+)\s*<\/strong>&nbsp;\s*([^<]+)/', $video, $titlematches);
	...
	// 构建目标网站的 URL
   $sourceUrl  = "https://www.xvideos.com/channels-index";
...
if (isset($matches[1])) {
    ...
    // 利用正则表达式进一步分割内容
    preg_match('/cats-links hidden(.*?)ul>/s', $content, $mainMatches);
    ...
    if (isset($mainMatches[1]) && isset($regionMatches[1]) && isset($rankMatches[1])) {
        ...
        foreach ($regionItems as $item) {
            // 提取 id 和 title
            preg_match('/href="\/channels-index(.*?)">(.*?)<\/a>/', $item, $matches);
            $id = isset($matches[1]) ? $matches[1] : '';
            $title = isset($matches[2]) ? $matches[2] : '';
            // 添加到结果数组
            if (!empty($title)) {
...
}  elseif (isset($_GET['channels']) && isset($_GET['page'])) {
...
// 获取给定链接的 JSON 数据
$url =  "https://www.xvideos.com/channels$channels/videos/best/$page";
